...
#define O_i8(v)		v.B.L = M_READ_OPERAND(PC.A); v.B.H = 0; PC.W.PC++

#define O_i16(v)	v.B.L = M_READ_OPERAND(PC.A); v.B.H = M_READ_OPERAND(PC.A+1); PC.W.PC += 2
...
#define O_a(a)		a.B.L = M_READ_OPERAND(PC.A); a.B.H = M_READ_OPERAND(PC.A+1); a.B.B = DB; PC.W.PC += 2;

#define O_al(a)		a.B.L = M_READ_OPERAND(PC.A); a.B.H = M_READ_OPERAND(PC.A+1); a.B.B = M_READ_OPERAND(PC.A+2); PC.W.PC += 3

#define O_d(a)		if (D.B.L) cpu_cycle_count++;			\
			a.A = D.W + M_READ_OPERAND(PC.A); a.B.B = 0; PC.W.PC++

#ifdef SHORT_X

#define O_dix(a)	if (D.B.L) cpu_cycle_count++;					\
			atmp.A = D.W + M_READ_OPERAND(PC.A); atmp.B.B = 0; PC.W.PC++;		\
			a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = DB;	\
			atmp.A = a.A;							\
			a.A += Y.W;							\
...
#define O_dixl(a)	if (D.B.L) cpu_cycle_count++;							\
			atmp.A = D.W + M_READ_OPERAND(PC.A); atmp.B.B = 0; PC.W.PC++;				\
			a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = M_READ_OPERAND(atmp.A+2);	\
			a.A += Y.W

#define O_dxi(a)	if (D.B.L) cpu_cycle_count++;					\
			atmp.A = D.W + M_READ_OPERAND(PC.A) + X.W; atmp.B.B = 0; PC.W.PC++;	\
			a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = DB

#ifdef NATIVE_MODE

#define O_dxx(a)	if (D.B.L) cpu_cycle_count++;				\
			a.W.L = (M_READ_OPERAND(PC.A) + D.W + X.W); a.B.B = 0; PC.W.PC++

#define O_dxy(a)	if (D.B.L) cpu_cycle_count++;				\
			a.W.L = (M_READ_OPERAND(PC.A) + D.W + Y.W); a.B.B = 0; PC.W.PC++

#ifdef SHORT_X

#define O_axx(a)	a.B.L = M_READ_OPERAND(PC.A); a.B.H = M_READ_OPERAND(PC.A+1); a.B.B = DB;	\
			atmp.A = a.A; a.A += X.W;					\
			if (atmp.B.H != a.B.H) cpu_cycle_count++;			\
			PC.W.PC+=2

#define O_axy(a)	a.B.L = M_READ_OPERAND(PC.A); a.B.H = M_READ_OPERAND(PC.A+1); a.B.B = DB;	\
			atmp.A = a.A; a.A += Y.W;					\
			if (atmp.B.H != a.B.H) cpu_cycle_count++;			\
			PC.W.PC+=2
#else
...
#define O_dxx(a)	if (D.B.L) cpu_cycle_count++;					\
			a.W.L = (M_READ_OPERAND(PC.A) + D.W + X.W); a.B.H = 0; a.B.B = 0; PC.W.PC++;

#define O_dxy(a)	if (D.B.L) cpu_cycle_count++;					\
			a.W.L = (M_READ_OPERAND(PC.A) + D.W + Y.W); a.B.H = 0; a.B.B = 0; PC.W.PC++;
...
#define O_alxx(a)	a.B.L = M_READ_OPERAND(PC.A); a.B.H = M_READ_OPERAND(PC.A+1); a.B.B = M_READ_OPERAND(PC.A+2); a.A += X.W; \
			PC.W.PC += 3;

#define O_pcr(a)	wtmp.B.L = M_READ_OPERAND(PC.A); PC.W.PC++;		\
			a.W.L = PC.W.PC + (offset_s) wtmp.B.L; a.B.B = PC.B.PB;

#define O_pcrl(a)	wtmp.B.L = M_READ_OPERAND(PC.A); wtmp.B.H = M_READ_OPERAND(PC.A+1); PC.W.PC += 2;	\
			a.W.L = PC.W.PC + (offset_l) wtmp.W; a.B.B = PC.B.PB;

#define O_ai(a)		atmp.B.L = M_READ_OPERAND(PC.A); atmp.B.H = M_READ_OPERAND(PC.A+1); atmp.B.B = 0; PC.W.PC += 2;	\
			a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = 0;

#define O_ail(a)	atmp.B.L = M_READ_OPERAND(PC.A); atmp.B.H = M_READ_OPERAND(PC.A+1); atmp.B.B = 0; PC.W.PC += 2;	\
			a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = M_READ_OPERAND(atmp.A+2)

#define O_di(a)		if (D.B.L) cpu_cycle_count++;					\
			atmp.A = M_READ_OPERAND(PC.A) + D.W; PC.W.PC++;				\
			a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = DB
...
#define O_axi(a)	atmp.B.L = M_READ_OPERAND(PC.A); atmp.B.H = M_READ_OPERAND(PC.A+1);				\
			atmp.A += X.W; atmp.B.B = PC.B.PB;						 \
			PC.W.PC += 2; a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = PC.B.PB

#define O_sr(a)		a.W.L = M_READ_OPERAND(PC.A) + S.W; a.B.B = 0; PC.W.PC++

#define O_srix(a)	atmp.W.L = M_READ_OPERAND(PC.A) + S.W; atmp.B.B = 0; PC.W.PC++;				\
			a.B.L = M_READ_OPERAND(atmp.A); a.B.H = M_READ_OPERAND(atmp.A+1); a.B.B = DB; a.A += Y.W
