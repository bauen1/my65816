...
void CPU_debug(void) {
	...
	switch (mode) {
        case IMM8:
            sprintf( operands, "#$%02X", M_READ_OPERAND(PC.A+1) );
            break;

        case IMM:
            operand = M_READ_OPERAND(PC.A+1) | (M_READ_OPERAND(PC.A+2)<<8);
        ...
        case ABSL:
            operand = M_READ_OPERAND(PC.A+1) | (M_READ_OPERAND(PC.A+2)<<8) | (M_READ_OPERAND(PC.A+3)<<16);
            ea = operand;
